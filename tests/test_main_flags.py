...
def test_run_profile_repo_identity_used_without_flag(tmp_path):
    pytest.importorskip("yaml")
    vault = tmp_path / "vault"
    vault.mkdir()
    (vault / ".obsidian").mkdir()
...
def test_obsidian_repo_identity_missing_value_exits_two(tmp_path):
    result = _graphify(["--obsidian", "--repo-identity"], cwd=tmp_path)
