...
- [ ] **OBSCMD-01**: Plan 00 — migrate `_uninstall_commands()` in `__main__.py:153` from hardcoded filename whitelist to directory-scan (reads `graphify/commands/*.md` at runtime). Existing Phase-11 commands remain installed; refactor is transparent
...
- [ ] **ARGUE-12** `[P2]`: Persona memory across rounds (persistent perspective identity) — each persona retains its prior claims across the debate to avoid contradiction
- [ ] **ARGUE-13** `[P2]`: Clash/rumble/domain intensity scoring annotates the transcript with conflict density metrics for post-hoc review

### Conversational Graph Chat (Phase 17)

- [ ] **CHAT-01**: New MCP tool `chat(query: str, session_id: str | None = None)` runs a two-stage pipeline structurally enforced (Stage 1 tool-call only → Stage 2 compose from results only)
- [ ] **CHAT-02**: Stage 1 translates NL query to a tool-call sequence over `_score_nodes` / `_bfs` / `_dfs` / `_bidirectional_bfs` / `_get_community` / `_connect_topics`
- [ ] **CHAT-03**: Stage 2 composes a structured narrative packet from the tool results; `serve.py` makes zero LLM calls (skill or calling agent renders the human-facing narrative)
- [ ] **CHAT-04**: Every claim paired with `{node_id, label, source_file}` in `meta["citations"]`; empty or fabricated cites rejected by post-process grep (Pitfall 5 mitigation)
- [ ] **CHAT-05**: Empty graph-query results return templated fuzzy suggestions ("did you mean X, Y?") — NEVER fabricate node names
...
- [ ] **CHAT-08**: Session history scoped per `session_id`; no cross-session memory (privacy by default)
- [ ] **CHAT-09**: D-02 MCP envelope + response size cap (≤ 500 tokens narrative, meta envelope unlimited)
