...
**Status:** SHIPPED (2026-05-07) — 28/28 requirements satisfied across 10 phases (63, 64, 65, 66, 67, 68, 69, 70, 70.1, 70.2). Archived during `/gsd-complete-milestone`.
...
- [x] **CCONF-01**: Concept↔code edges emit per-edge `confidence_score ∈ [0.0, 1.0]` from LLM scoring (replaces uniform `1.0` baseline shipped in v1.10–v1.12).
- [x] **CCONF-02**: INFERRED edges include an `evidence` field carrying the textual basis for the assigned score.
- [x] **CCONF-03**: Confidence cache lives in a separate namespace keyed on `prompt_version + model + edge_signature` (independent from the existing `extract.py` file-hash cache); bumping `prompt_version` invalidates only confidence entries.
- [x] **CCONF-04**: GRAPH_REPORT.md gains a calibration self-check section that flags suspicious score distributions (e.g., >70% of scores clustered in 0.85±0.05).
- [x] **CCONF-05**: `validate.py` enforces "optional on read, required on write" with a new `schema_version` field; a frozen v1.10–v1.12 legacy fixture must pass read validation in `tests/`.
...
- [x] **CFED-02**: Federation is deterministic — namespaces all node IDs as `{repo}::{id}`; merges only when multi-signal evidence (label + shared neighborhood + source-path overlap) all agree. No embeddings, no LLM arbitration.
- [x] **CFED-03**: A federation manifest records per-repo provenance for every merged concept (which repos contributed, which signals matched).
...
- [x] **CQUERY-02**: Backward compatibility — callers that omit the new parameters receive results identical to the v1.12 implementation.
