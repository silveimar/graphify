...
When there is almost nothing to extract yet, use: `graphify elicit` (see `docs/ELICITATION.md`). Use full `/graphify` once you have files to ingest.
...
if isinstance(result, MergePlan):
    ...
    print(f'Obsidian vault: {total} writes in graphify-out/obsidian/ '
...
This starts a stdio MCP server that exposes tools including `query_graph`, `get_node`, `get_neighbors`, `get_community`, `god_nodes`, `graph_stats`, `shortest_path`, `entity_trace`, `concept_code_hops` (concept↔code via `implements`; complements temporal `/trace`), and others — see `graphify/mcp_tool_registry.py`.
