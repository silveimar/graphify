...
- If graphify-out/obsidian/ exists, navigate the MOC notes in its Graphify-owned v1.8 subtree and follow [[wikilinks]] between nodes instead of reading raw files
...
def _pop_global_write_into_vault(argv: list[str]) -> tuple[list[str], bool]:
    ...
    Multiple consecutive occurrences collapse to True."""
    out = list(argv)
    flag = False
...
def _check_vault_cwd_gate(
    ...
    *,
    ...
    if not is_obsidian_vault(cwd):
        return "n/a"
    # Explicit routing flag wins — silent precedence, no notice emitted.
...
def main() -> None:
    ...
    sys.argv, g_write_into_vault = _pop_global_write_into_vault(sys.argv)  # VCWD-04
    ...
    if len(sys.argv) < 2 or sys.argv[1] in ("-h", "--help"):
        ...
        print("  run [path] [--router]     detect → AST extract only (Phase 12); does not write graph.json")
        print("                            For build + elicitation sidecar merge: watch rebuild, update-vault,")
        print("                            or /graphify skill pipeline — see docs/ELICITATION.md")
        print("    --repo-identity <slug>  override profile/fallback repo identity for generated artifacts")
    ...
    if cmd == "install":
        ...
        lv_write_into_vault, rest = _strip_write_into_vault_from_tokens(rest)
        gate = _check_vault_cwd_gate(
            cmd,
            has_explicit_route=bool(
                g_vault_exp or g_vault_list or lv_vault or lv_vlist
        ...
        _p_vp.add_argument("--vault", required=False, default=None, help="Path to the Obsidian vault directory (optional when invoked from a vault CWD)")
        _p_vp.add_argument(
            "--threshold",
            type=int,
            default=3,
